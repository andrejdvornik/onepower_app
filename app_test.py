...
def plot_card(fig, key=None, title=None):
    config = {'responsive': False, 'staticPlot': True}
    fig.update_layout(
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        margin=dict(l=5, r=5, t=15 if title else 10, b=10),
    )
    if title:
        st.markdown(f'**{title}**')
    st.plotly_chart(fig, width='stretch', height='stretch', key=key, config=config)
                ...
                grid_items.append(
                    {
                        'type': 'combined_pk',
                        'label': 'Power Spectra',
                        'selected_outputs': selected_outputs.copy(),
                    }
                )
